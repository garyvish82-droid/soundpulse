#!/usr/bin/env node
/**
 * SoundPulse MCP Server
 * Exposes 4 tools for AI agents to query SoundCloud analytics
 */
...
server.tool(
  ...
  async ({ user_id, track_id, timeframe }) => {
    ...
    if (!data || data.length === 0) {
      return { content: [{ type: "text", text: `No insights found for user ${user_id}. Run the insight agent first.` }] };
    }
...
server.tool(
  ...
  {
    user_id: z.string().describe("SoundCloud user ID"),
    goal: z.string().optional().describe("Optional goal: 'more reposts', 'grow Germany', 'improve completion rate', 'grow plays', 'grow followers'"),
  },
  async ({ user_id, goal }) => {
    let query = supabase
      .from("ai_insights")
    ...
    if (error) throw new Error(`DB error: ${error.message}`);
    if (!data || data.length === 0) {
      return { content: [{ type: "text", text: `No strategy found for user ${user_id}. Run the strategy agent first.` }] };
    }
...
server.tool(
  ...
  async ({ user_id }) => {
    ...
    if (error) throw new Error(`DB error: ${error.message}`);
    if (!data || data.length === 0) {
      return { content: [{ type: "text", text: `No alerts found for user ${user_id}. Run the anomaly agent first.` }] };
    }
...
async function main() {
  ...
  console.error("SoundPulse MCP server running on stdio");
}
